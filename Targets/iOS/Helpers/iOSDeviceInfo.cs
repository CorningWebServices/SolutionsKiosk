...
namespace Mediando.iOS
{
	...
	{
		public IOSDevices GetDeviceModel (string hardware)
		{
			if (hardware.StartsWith ("iPhone")) {
				if (hardware.StartsWith ("iPhone4,"))
					return IOSDevices.IPhone4S;

				if (hardware.StartsWith ("iPhone5,"))
					return IOSDevices.IPhone5;

				if (hardware.StartsWith ("iPhone6,"))
					return IOSDevices.IPhone5S;

				if (hardware == "iPhone7,1")
					return IOSDevices.IPhone6Plus;

				if (hardware == "iPhone7,2")
					return IOSDevices.IPhone6;
			...
			if (hardware.StartsWith ("iPad")) {
				if (hardware == "iPad2,1" || hardware == "iPad2,2" || hardware == "iPad2,3" || hardware == "iPad2,4")
					return IOSDevices.IPad2;
				
				if (hardware.StartsWith ("iPad3,"))
					return IOSDevices.IPadWithRetina;
				
				if (hardware == "iPad4,1" || hardware == "iPad4,2" || hardware == "iPad4,3" || hardware == "iPad5,3" || hardware == "iPad5,4")
					return IOSDevices.IPadAir;

				if (hardware == "iPad2,5" || hardware == "iPad2,6" || hardware == "iPad2,7"
				    || hardware == "iPad4,4" || hardware == "iPad4,5" || hardware == "iPad4,6" || hardware == "iPad4,7" || hardware == "iPad4,8" || hardware == "iPad4,9")
					return IOSDevices.IPadMini;

			}

			if (hardware == "i386" || hardware == "x86_64")
				return IOSDevices.Simulator;
			else
				return IOSDevices.UnKnown;
