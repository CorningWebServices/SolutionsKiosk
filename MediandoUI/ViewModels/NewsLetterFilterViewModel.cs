...
namespace MediandoUI
{
	...
	public class NewsLetterFilterViewModel : BaseViewModel
	{
		...
		public NewsLetterFilterViewModel (FilterTypes filter)
		{
				...
				if (CurrentFilter == FilterTypes.Categories) {
					...
					foreach (var item in categories) {
						source.Add (new MultiSelectSource (item.CategoryCode, item.CategoryName,
							item.CategoryCode == GlobalVariables.NewsCategory ? true : false));
					}
				} 
				else 
				{
					...
					foreach (var item in categories) {
						source.Add (new MultiSelectSource (item.Language, item.Name,
							item.Language == GlobalVariables.NewsLanguage ? true : false));
